...
#		while ite < n:
#			if str(a_id[ite]["albumid"]) == request.args.get('albumid'):
#				error404 = False
#			ite = ite + 1
...
#		return  request.args.get('albumid')
                ...
                picid_list = get_pic_list()
		if results not in picid_list:
			abort(404)
		else:
	        	options = {
	                        "edit": True,
...
def get_pic_list():
	cur = db.cursor()
	cur.execute("SELECT C.picid FROM Contain C, Photo P WHERE P.picid = C.picid");
